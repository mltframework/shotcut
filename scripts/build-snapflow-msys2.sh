...
# This script builds snapflow and many of its dependencies.
# It can accept a configuration file, default: build-snapflow.conf
...
CONFIGFILE=build-snapflow.conf
...
function usage {
  echo "Usage: $0 [-c config-file] [-h] [-s] [-v snapflow-version]"
  echo "Where:"
  echo -e "\t-c config-file\tDefaults to $CONFIGFILE"
  echo -e "\t-s\t\tbuild SDK"
  echo -e "\t-v snapflow-version\t\tSet the Snapflow version; defaults to $SNAPFLOW_VERSION"
}
...
function set_globals {
  ...
  REPOLOCS[4]="https://github.com/mltframework/snapflow.git"
...
#  REPOLOCS[8]="https://bitbucket.org/dandennedy/bigsh0t.git"
  ...
  REVISIONS[4]=""
  if test 0 = "$SNAPFLOW_HEAD" -a "$SNAPFLOW_REVISION" ; then
    REVISIONS[4]="$SNAPFLOW_REVISION"
  fi
  REVISIONS[5]=""
  if test 0 = "$LADSPA_HEAD" -a "$LADSPA_REVISION" ; then
    REVISIONS[5]="$LADSPA_REVISION"
  fi
  REVISIONS[6]="$NV_CODEC_REVISION"
  REVISIONS[7]="" # AMF
  REVISIONS[8]=""
  ...
  if test 1 = "$AUTO_APPEND_DATE" ; then
    ...
    FINAL_INSTALL_DIR="$INSTALL_DIR/Snapflow"
  fi
  ...
  CXXFLAGS_[1]="${CFLAGS_[1]} -D_XOPEN_SOURCE=700"
  ...
  #####
  # snapflow
  CONFIG[4]="cmake -G Ninja -D CMAKE_INSTALL_PREFIX=$FINAL_INSTALL_DIR -DCMAKE_PREFIX_PATH=$QTDIR -D SNAPFLOW_VERSION=$SNAPFLOW_VERSION $CMAKE_DEBUG_FLAG"
  CFLAGS_[4]="$ASAN_CFLAGS $CFLAGS"
  LDFLAGS_[4]="$ASAN_LDFLAGS $LDFLAGS"
  BUILD[4]="ninja -j $MAKEJ"
  INSTALL[4]="install_snapflow"
  ...
  if [ "$TARGET_ARCH" = "clangarm64" ]; then
    ...
    CONFIG[9]="${CONFIG[9]} -D GGML_AVX2=OFF -D GGML_BLAS=ON -D GGML_BLAS_VENDOR=OpenBLAS"
  fi
  CFLAGS_[9]=$CFLAGS
  LDFLAGS_[9]=$LDFLAGS
  ...
  #####
  # opencv
  CONFIG[15]="cmake -B build -G Ninja -D CMAKE_INSTALL_PREFIX=$FINAL_INSTALL_DIR -D CMAKE_CXX_STANDARD=14 -D BUILD_LIST=tracking -D OPENCV_GENERATE_PKGCONFIG=YES -D OPENCV_EXTRA_MODULES_PATH=../opencv_contrib/modules -D WITH_OPENMP=ON $CMAKE_DEBUG_FLAG"
  [ "$TARGET_ARCH" = "clangarm64" ] && CONFIG[15]="${CONFIG[15]} -DWITH_LAPACK=OFF -DWITH_OPENBLAS=OFF"
  CFLAGS_[15]="$CFLAGS"
  LDFLAGS_[15]="$LDFLAGS"
...
function install_snapflow {
  cmd ninja install
  cmd install -c COPYING "$FINAL_INSTALL_DIR"
  cmd install -c packaging/windows/snapflow.iss "$FINAL_INSTALL_DIR"/..
  cmd sed -i "s/YY.MM.DD/$SNAPFLOW_VERSION/" "$FINAL_INSTALL_DIR"/../snapflow.iss
...
function get_subproject {
  ...
  if test "git" = "$REPOTYPE" ; then
      ...
      # Apply FFmpeg patch from the snapflow subproject
      if test "snapflow" = "$1" ; then
          PATCH_FILE="$SOURCE_DIR/snapflow/scripts/ffmpeg8-scale_d3d-refleak.patch"
...
function deploy
{
  ...
  if [ "$DEBUG_BUILD" = "1" -o "$SDK" = "1" ]; then
    [ "$TARGET_ARCH" != "clangarm64" ] && cmd cp -p "$SOURCE_DIR"/snapflow/drmingw/x64/bin/*.{dll,yes} .
...
#    cmd cp -p /${TARGET_ARCH}/lib/cmake/fftw3q/*.cmake lib/cmake/fftw3q/
  ...
  # colortap is not used by Snapflow and no plans to include it, but
  ...
  if [ "$SDK" = "1" ]; then
    ...
    cmd cd ..
    cmd tar -cJf snapflow-sdk.txz Snapflow
    fi
  popd
}
