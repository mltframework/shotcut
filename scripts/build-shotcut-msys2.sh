...
#  REPOLOCS[8]="https://bitbucket.org/dandennedy/bigsh0t.git"
  ...
  #####
  # shotcut
  CONFIG[4]="$QTDIR/bin/qmake -r MLT_PATH=$FINAL_INSTALL_DIR PREFIX=$FINAL_INSTALL_DIR $QMAKE_DEBUG_FLAG $QMAKE_ASAN_FLAGS"
  CFLAGS_[4]=$CFLAGS
  LDFLAGS_[4]=$LDFLAGS
...
function deploy
{
  ...
  if [ "$DEBUG_BUILD" != "1" -o "$SDK" = "1" ]; then
    cmd cp -p "$QTDIR"/bin/Qt5{Core,Gui,Multimedia,Network,OpenGL,Qml,QmlModels,QmlWorkerScript,Quick,QuickWidgets,Sql,Svg,WebSockets,Widgets,WinExtras,Xml}.dll .
  fi
  if [ "$DEBUG_BUILD" = "1" -o "$SDK" = "1" ]; then
