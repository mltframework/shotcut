...
#  REPOLOCS[8]="https://bitbucket.org/dandennedy/bigsh0t.git"
  ...
  #####
  # glaxnimate
  CONFIG[13]="cmake -G Ninja -DCMAKE_PREFIX_PATH=$QTDIR -DCMAKE_INSTALL_PREFIX=$FINAL_INSTALL_DIR"
  if [ "$DEBUG_BUILD" = "1" ]; then
    CONFIG[13]="${CONFIG[13]} -DCMAKE_BUILD_TYPE=RelWithDebInfo"
  else
    CONFIG[13]="${CONFIG[13]} -DCMAKE_BUILD_TYPE=Release"
  fi
