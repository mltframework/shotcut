...
function set_globals {
  ...
  if test 1 = "$AUTO_APPEND_DATE" ; then
    ...
    FINAL_INSTALL_DIR="$INSTALL_DIR/Snapflow/Snapflow.app"
  fi
  ...
  #####
  # snapflow
  CONFIG[7]="cmake -G Ninja -D CMAKE_PREFIX_PATH=$QTDIR -D SNAPFLOW_VERSION=$SNAPFLOW_VERSION $CMAKE_DEBUG_FLAG"
  if test "$TARGET_OS" = "Darwin" ; then
  ...
  if [ "$TARGET_OS" = "Darwin" ]; then
    ...
    INSTALL[7]="install_snapflow_linux"
  fi
...
function install_snapflow_linux {
  cmd ninja install
  cmd install -p -c COPYING "$FINAL_INSTALL_DIR"
  cmd install -p -c "$QTDIR"/translations/qt_*.qm "$FINAL_INSTALL_DIR"/share/snapflow/translations
  cmd install -p -c "$QTDIR"/translations/qtbase_*.qm "$FINAL_INSTALL_DIR"/share/snapflow/translations
...
Basic Build Instructions for Snapflow
...
to make Snapflow daily builds. It is the authoritative install reference:
  src/snapflow/scripts/build-snapflow.sh

We cannot cover how to build all of Snapflow's dependencies from scratch here.
...
As for Snapflow itself:
  cd snapflow; mkdir build ; cd build ; cmake .. ; cmake --build .
...
dependencies need to be put into the correct locations in Snapflow.app,
...
yourself, you do not need to do that. You can just let Snapflow use
...
function deploy_mac
{
  ...
  # Change to right directory
  log Changing directory to snapflow
  cmd cd snapflow || die "Unable to change to directory snapflow"

  BUILD_DIR="./Snapflow.app/Contents"

  # copy Qt translations
  cmd mkdir -p "$BUILD_DIR/Resources/snapflow/translations"
  # try QTDIR first
  if [ -d "$QTDIR/translations" ]; then
    cmd cp -p "$QTDIR"/translations/qt_*.qm "$BUILD_DIR/Resources/snapflow/translations/"
    cmd cp -p "$QTDIR"/translations/qtbase_*.qm "$BUILD_DIR/Resources/snapflow/translations/"
  # try Qt Creator after that
  elif [ -d "/Applications/Qt Creator.app/Contents/Resources/translations" ]; then
    cmd cp -p "/Applications/Qt Creator.app/Contents/Resources/translations/"qt_*.qm "$BUILD_DIR/Resources/snapflow/translations/"
    cmd cp -p "/Applications/Qt Creator.app/Contents/Resources/translations/"qtbase_*.qm "$BUILD_DIR/Resources/snapflow/translations/"
  fi
  # copy Snapflow translations
  cmd cp translations/*.qm "$BUILD_DIR/Resources/snapflow/translations/"
  ...
  for exe in MacOS/Snapflow MacOS/melt MacOS/ffmpeg MacOS/ffplay MacOS/ffprobe MacOS/glaxnimate; do
    fixlibs "$exe"
  ...
  cmd cp -a "$FINAL_INSTALL_DIR"/share/snapflow/whisper_models Resources/snapflow
  ...
  if [ "$SDK" = "1" ]; then
    ...
    cmd rm -rf "$INSTALL_DIR"/Snapflow
    cmd mv snapflow/Snapflow.app "$INSTALL_DIR"/Snapflow
    clean_dirs
    pushd "$INSTALL_DIR"
    log Copying src
    cmd cp -a "$SOURCE_DIR" Snapflow
    log Copying includes
    cmd cp -a "$FINAL_INSTALL_DIR"/include Snapflow/Contents/Frameworks
    log Copying pkg-config files
    cmd mkdir -p Snapflow/Contents/Frameworks/lib 2> /dev/null
    cmd cp -a "$FINAL_INSTALL_DIR"/lib/pkgconfig Snapflow/Contents/Frameworks/lib
    log Symlinking libs
    pushd Snapflow/Contents/Frameworks
  ...
  if [ "$ACTION_ARCHIVE" = "1" ]; then
    if [ "$SDK" = "1" ]; then
      log Making archive
      cmd tar -cJvf snapflow.txz Snapflow
      [ "$ACTION_CLEANUP" = "1" ] && cmd rm -rf Snapflow
      popd
    else
      # build DMG
      ...
      cmd mkdir staging
      cmd mv snapflow/Snapflow.app staging
      cmd ln -s /Applications staging
      cmd cp snapflow/COPYING staging

      log Making disk image
      dmg_name="$INSTALL_DIR/unsigned.dmg"
      cmd rm "$dmg_name" 2>/dev/null
      sync
      cmd hdiutil create -fs HFS+ -srcfolder staging -volname Snapflow -format UDBZ -size 1500m "$dmg_name"
...
function create_startup_script {
  ...
  TMPFILE=`mktemp -t build-snapflow.env.XXXXXXXXX`
...
# LADSPA_PATH set, and Snapflow only needs the supplied SWH plugins.
...
End-of-exe-wrapper
  ...
  cat > $TMPFILE <<End-of-snapflow-wrapper
#!/bin/sh
# Set up environment
# Run this instead of trying to run bin/snapflow. It runs snapflow with the correct environment.
CURRENT_DIR=\$(readlink -f "\$0")
INSTALL_DIR=\$(dirname "\$CURRENT_DIR")
export LD_LIBRARY_PATH="\$INSTALL_DIR/lib":\$LD_LIBRARY_PATH
export MLT_REPOSITORY="\$INSTALL_DIR/lib/mlt-7"
...
bin/snapflow "\$@"
End-of-snapflow-wrapper
  if test 0 != $? ; then
    die "Unable to create wrapper script"
  fi
  chmod 755 $TMPFILE || die "Unable to make wrapper script executable"
  cp $TMPFILE "$FINAL_INSTALL_DIR/snapflow" || die "Unable to create wrapper script - cp failed"
  ...
  VERSION_INFO=$(readlink -f Snapflow/Snapflow.app/versions)
  rm -f $VERSION_INFO
  for DIR in $SUBDIRS; do
  ...
  if [ "$ACTION_ARCHIVE" = "1" ]; then
    ...
    tarball="$INSTALL_DIR/snapflow.txz"
    ...
    if [ "$SDK" = "1" ]; then
      ...
      popd
      log Copying src
      cmd -rf Snapflow/Snapflow.app/src 2> /dev/null
      cmd cp -a "$SOURCE_DIR" Snapflow/Snapflow.app
    else
      cmd rm -rf Snapflow/Snapflow.app/include
      cmd rm Snapflow/Snapflow.app/lib/*.a
      cmd rm -rf Snapflow/Snapflow.app/lib/pkgconfig
      cmd rm -rf Snapflow/Snapflow.app/share/doc
      cmd rm -rf Snapflow/Snapflow.app/share/man
    fi
    cmd tar -C Snapflow -cJvf "$tarball" Snapflow.app
  fi
