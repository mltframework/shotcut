    ...
    , m_previousPercent(0)
{
    ...
    QDir outDir = QFileInfo(m_outputPath).dir();
...
void HtmlGeneratorJob::start()
{
    ...
    QString url("file://" + m_htmlFilePath);
...
void HtmlGeneratorJob::onAnimationFramesReady()
{
    ...
    emit progressUpdated(m_item, 50); // Frames generation is 50% done
...
void HtmlGeneratorJob::onReadyRead()
{
    ...
    do {
        ...
        if (msg.contains("frame=")) {
            ...
            auto match = frameRegex.match(msg);
            if (match.hasMatch()) {
                ...
                int percent = 50 + qRound((currentFrame * 50.0) / totalFrames); // 50-100%
...
void HtmlGeneratorJob::onHtmlGeneratorProgress(float progress)
{
    if (m_isGeneratingFrames) {
        // Map 0.0-1.0 to 0-50%
        int percent = qRound(progress * 50.0);
        emit progressUpdated(m_item, percent);
    }
}
