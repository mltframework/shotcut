    ...
    , ui(new Ui::HtmlGeneratorWidget)
{
    ...
    f.setFileName(QStringLiteral(":/resources/html/folded/css.txt"));
    f.open(QIODevice::ReadOnly);
    p.set(kCssProperty, f.readAll().toBase64().constData());
    f.close();
    f.setFileName(QStringLiteral(":/resources/html/folded/body.txt"));
    f.open(QIODevice::ReadOnly);
    p.set(kBodyProperty, f.readAll().toBase64().constData());
    f.close();
    p.set(kColorProperty, colorStringToResource(kTransparent).toLatin1().constData());
    ...
    f.setFileName(QStringLiteral(":/resources/html/gold_metal/css.txt"));
    f.open(QIODevice::ReadOnly);
    p.set(kCssProperty, f.readAll().toBase64().constData());
    f.close();
    f.setFileName(QStringLiteral(":/resources/html/gold_metal/body.txt"));
    f.open(QIODevice::ReadOnly);
    p.set(kBodyProperty, f.readAll().toBase64().constData());
    f.close();
    p.set(kColorProperty, colorStringToResource(kTransparent).toLatin1().constData());
...
{
    auto mltColor = p.get_color(kColorProperty);
...
<html>
  ...
  <body style="margin: 0; background: %2">
%3
  </body>
</html>
)");
...
void HtmlGeneratorWidget::on_imageButton_clicked()
{
    ...
    connect(
        ...
        Qt::QueuedConnection);
    QString url("file://" + txtFile->fileName());
    QSize size(qRound(MLT.profile().width() * MLT.profile().sar()), MLT.profile().height());
...
void HtmlGeneratorWidget::on_videoButton_clicked()
{
    ...
    int durationMs = static_cast<int>((durationFrames / static_cast<double>(MLT.profile().fps()))
                                      * 1000.0);
