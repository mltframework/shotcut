                                 ...
                                 QString *errorMessage) const
{
    ...
    stream
        ...
        << "    }\n\n"
           ...
           "        var entries = [];\n"
           "        var filterLines = [];\n"
           "        filterLines.push('<b>' + qsTr('Filter Title') + ':</b> ' + "
           "root.escapeHtml(root.filterTitle.length > 0 ? root.filterTitle : '-'));\n"
           "        if (root.filterDescription.length > 0)\n"
           "            filterLines.push('<b>' + qsTr('Filter Description') + ':</b> ' + "
           "root.escapeHtml(root.filterDescription));\n"
           "        entries.push(filterLines.join('<br/>'));\n"
           "\n"
           "        for (var i = 0; i < propertyNames.length; ++i) {\n"
           "            var name = propertyNames[i];\n"
           "            var title = (root.propertyTitles && root.propertyTitles[name]) ? "
           "root.propertyTitles[name] : name;\n"
           "            var description = (root.propertyDescriptions && "
           "root.propertyDescriptions[name]) ? root.propertyDescriptions[name] : '';\n"
           "            var entryLines = [];\n"
           "            entryLines.push('<b>' + qsTr('Parameter Title') + ':</b> ' + "
           "root.escapeHtml(title));\n"
           "            if (description.length > 0)\n"
           "                entryLines.push('<b>' + qsTr('Description') + ':</b> ' + "
           "root.escapeHtml(description));\n"
           "\n"
           "            var minValue = root.propertyMinimums ? root.propertyMinimums[name] : "
           "undefined;\n"
           "            var maxValue = root.propertyMaximums ? root.propertyMaximums[name] : "
           "undefined;\n"
           "            var hasMin = minValue !== undefined && minValue !== null && "
           "String(minValue) !== '';\n"
           "            var hasMax = maxValue !== undefined && maxValue !== null && "
           "String(maxValue) !== '';\n"
           "            if (hasMin && hasMax)\n"
           "                entryLines.push('<b>' + qsTr('Range') + ':</b> ' + "
           "root.escapeHtml(minValue) + ' - ' + root.escapeHtml(maxValue));\n"
           "\n"
           "            entries.push(entryLines.join('<br/>'));\n"
           "        }\n"
           "\n"
           "        return entries.join('<br/><br/>');\n"
           "    }\n\n"
           "    width: 360\n"
           ...
           "            onClicked: root.metadataHelpRequested(qsTr('Add-on Metadata'), "
           "root.metadataText())\n"
           "        }\n\n"
           "        Label {\n"
           "            text: qsTr('Preset')\n"
