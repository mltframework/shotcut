...
int sap_open_project(void *mainWindowHandle, const char *path)
{
    ...
    if (!mw || !path || !*path)
        return -1;
    const QString projectPath = QString::fromUtf8(path);
    int result = -1;
    QMetaObject::invokeMethod(
        mw,
        [mw, projectPath, &result]() { result = mw->open(projectPath) ? 0 : -1; },
        Qt::BlockingQueuedConnection);
    return result;
}
...
int sap_set_project_folder(void *mainWindowHandle, const char *folder)
{
    ...
    if (!mw)
        return -1;
    const QString projectFolder = folder ? QString::fromUtf8(folder) : QString();
    QMetaObject::invokeMethod(
        mw,
        [projectFolder]() { MLT.setProjectFolder(projectFolder); },
        Qt::BlockingQueuedConnection);
    return 0;
}
