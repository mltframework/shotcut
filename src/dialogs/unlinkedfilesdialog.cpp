...
void UnlinkedFilesDialog::on_tableView_doubleClicked(const QModelIndex &index)
{
    ...
    if (filenames.length() > 0) {
        ...
        if (hash == model->data(firstColIndex, MltXmlChecker::SnapflowHashRole)) {
        ...
        model->setData(secondColIndex, hash, MltXmlChecker::SnapflowHashRole);
...
bool UnlinkedFilesDialog::lookInDir(const QDir &dir, bool recurse)
{
    ...
    for (int row = 0; row < model->rowCount(); row++) {
        ...
        if (model->data(replacementIndex, MltXmlChecker::SnapflowHashRole).isNull())
            ++outstanding;
    }
    if (outstanding) {
        for (const auto &fileName :
            ...
            for (int row = 0; row < model->rowCount(); row++) {
                ...
                if (model->data(replacementIndex, MltXmlChecker::SnapflowHashRole).isNull()) {
                    ...
                    QString missingHash
                        = model->data(missingIndex, MltXmlChecker::SnapflowHashRole).toString();
                    if (hash == missingHash || fileName == missingInfo.fileName()) {
                        ...
                        model->setData(replacementIndex, hash, MltXmlChecker::SnapflowHashRole);
