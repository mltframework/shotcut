...
void HtmlGenerator::createNewPage()
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        ...
        QString webSocketUrl = page["webSocketDebuggerUrl"].toString();
...
void HtmlGenerator::startAnimationCapture()
{
    ...
    double frameInterval = 1000.0 / m_fps; // milliseconds per frame
...
void HtmlGenerator::handleAnimationFrame(const QJsonObject &result)
{
    ...
    QString base64Data = result["data"].toString();
    QByteArray imageData = QByteArray::fromBase64(base64Data.toUtf8());

    // Save frame with zero-padded filename
    QString frameNumber = QString("%1").arg(m_currentFrame, 4, 10, QLatin1Char('0'));
    QString filename = QDir(m_outputPath).filePath(QString("frame_%1.png").arg(frameNumber));
    ...
    if (m_currentFrame < m_totalFrames) {
        ...
        qint64 targetTime = static_cast<qint64>(m_currentFrame * frameInterval);
        qint64 currentTime = m_animationElapsed.elapsed();

        int delay = static_cast<int>(std::max(0LL, targetTime - currentTime));
...
void HtmlGenerator::completeAnimationCapture()
{
    ...
    qDebug() << QString("Animation frames saved to %1/").arg(m_outputPath);
